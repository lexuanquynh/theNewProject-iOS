...
public class Downloader<T>: AbstractDownloader {
    ...
    public func download(with urlString: String?, completionHandler: @escaping (DownloadCompletionHandler<T>), placeholder: T?) {
        ...
        if let image = getCachedDataFrom(urlString: urlString) {
            ...
            if getDataTaskFrom(urlString: urlString) != nil {
                return
            }
            ...
            if #available(iOS 13.0, *) {
                cache = URLCache(memoryCapacity: 10_000_0000, diskCapacity: 1_000_000_0000, directory: diskCacheURL)
            } else {
                // Fallback on earlier versions
                cache = URLCache(memoryCapacity: 10_000_0000, diskCapacity: 1_000_000_0000, diskPath : "DownloadCache") // path on disk where the cache data is stored
